...
public class SwordzDude : MonoBehaviour {
	...
	void Start ()
	{
        ...
        if (chargeDecal != null)
            chargeDecal.gameObject.SetActive(false);
	...
	IEnumerator doSwing(SwingEvent swingEvent)
	{
		...
		*/
		...
		if(swingEvent.animation != null)
		{
			...
			animated.GetComponent<Animation>().Play(animName, PlayMode.StopAll);//CrossFade(animName);
		}
	...
	public void OnKill()
	{
		...
		if(corpse != null && animated != null)
		{
			...
			corpse.GetComponent<Rigidbody>().AddForce(launchVec);
			corpse.GetComponent<Rigidbody>().AddTorque(Random.insideUnitSphere * 1000f);
	...
	public void OnStopAttack()
	{
		dude.velocity = Vector3.zero;
		animated.GetComponent<Animation>().Stop();
		animated.GetComponent<Animation>().Play("bjorn_idle");
	...
	public IEnumerator OnStun(float stunDuration)
	{
		...
		{
			animated.GetComponent<Animation>()["bjorn_die"].speed = 1;
			animated.GetComponent<Animation>().Play("bjorn_die");
		...
		{
			// get up
			animated.GetComponent<Animation>()["bjorn_die"].normalizedTime = 1.0f;
			animated.GetComponent<Animation>()["bjorn_die"].speed = -1;
			animated.GetComponent<Animation>().Play("bjorn_die");
