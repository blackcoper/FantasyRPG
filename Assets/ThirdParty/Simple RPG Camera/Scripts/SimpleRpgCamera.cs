...
public class SimpleRpgCamera : MonoBehaviour
{
	...
	private List<UnityEngine.Material> _fadedMats = new List<UnityEngine.Material>();
	private List<UnityEngine.Material> _activeFadedMats = new List<UnityEngine.Material>();
	...
	void Update()
	{
		...
		foreach(UnityEngine.Material mat in _fadedMats)
		{
	...
	void LateUpdate()
	{
		if(target && _focalPoint)
		{
			...
			foreach(RaycastHit alphaHit in hits)
			{
				...
				if(hitRenderer)
				{
                    UnityEngine.Material[] mats = hitRenderer.materials;

					foreach(UnityEngine.Material mat in mats)
					{
						Color c = mat.color;
						c.a = collisionAlpha;
